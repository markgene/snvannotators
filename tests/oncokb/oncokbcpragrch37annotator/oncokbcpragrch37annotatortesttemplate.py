"""Test template for OncokbCpraGrch37Annotator class."""
...
class OncokbCpraGrch37AnnotatorTestTemplate:
    """Test template for OncokbCpraGrch37Annotator class."""
    ...
    def test_variant_exist(self):
        if self.variant_exist is True:
            self.assertTrue(self.indicator_query_resp.variant_exist)
        elif self.variant_exist is False:
            self.assertFalse(self.indicator_query_resp.variant_exist)
        else:
            raise ValueError("variant_exist must be True or False")

    def test_vus(self):
        if self.vus is True:
            self.assertTrue(self.indicator_query_resp.vus)
        elif self.vus is False:
            self.assertFalse(self.indicator_query_resp.vus)
        else:
            raise ValueError("vus must be True or False")
